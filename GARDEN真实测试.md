...
选择性（selectivity）是**针对每个查询**而言的：该查询的过滤条件命中数据库的多大比例。本节给出 EM 与 R 查询的真实选择率。

**EM 查询（等值过滤）**：同一 label 的所有查询选择率相同，且因等值查询的特性，"某值的库内占比"即"按该值查询的选择率"：

| label | 该 label 查询数 | 单查询命中数 | 单查询选择率 |
|-------|----------------|--------------|--------------|
| 1 | 5,263 | 52,817 | 52.8% |
| 2 | 3,037 | 29,941 | 29.9% |
| 3 | 1,187 | 12,040 | 12.0% |
...
查询的 label 分布与库内值分布一致（数据集按值分布正比采样查询），故大量查询落在高选择性的 label 1/2，少量落在低选择性的 label 5-7。

**R 查询（区间过滤）**：每条查询区间宽度不同，选择率是一个连续分布，需逐条计算：

```
min 0.25%  |  p10 16.6%  |  p25 27.7%  |  p50 43.8%  |  p75 60.8%  |  p90 73.8%  |  max 95.1%
```

按 GARDEN 路由阈值分桶，10,000 条 R 查询落在：

| 路径 | 查询数 | 占比 |
|------|--------|------|
| 候选 < 10k（暴力） | 413 | 4.1% |
| 10k–80%（子图搜索） | 9,068 | **90.7%** |
| 选择率 > 80%（全量图） | 519 | 5.2% |

**关键观察**：R 查询 90.7% 落在 GARDEN 的子图搜索区间、选择率中位数约 43.8%。这说明在真实负载下，GARDEN 的子图路径是**高频命中的主战场**，而非边角场景——这是"中等选择性是过滤检索主要场景"的直接证据。
